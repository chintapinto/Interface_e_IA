...
from ui_components import CameraConfigDialog, ROISelector
...
class MainWindow(QMainWindow):
    ...
    def update_button_states(self):
        ...
        self.edit_cam_button.setEnabled(len(self.get_selected_rows()) == 1)
        self.remove_cam_button.setEnabled(has_selection)
        if not has_selection:
            self.start_button.setEnabled(False)
            self.stop_button.setEnabled(False)
        else:
            can_start = any(self.camera_table.item(row, 1).text() == "Inativo" for row in self.get_selected_rows())
            can_stop = any(self.camera_table.item(row, 1).text() == "Ativo" for row in self.get_selected_rows())
